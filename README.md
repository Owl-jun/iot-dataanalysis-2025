...
- [노트북](./DAY03/mldl01_선형회귀%20다항회귀.ipynb)
...
- 선형 방정식으로 학습
    ...
    - z = (a * weight) + (b * len) + (c * 대각길이) + (d * height) + (e * depth) + f
    - z = 0~1

- `시그모이드함수` - z가 아주 큰 음수일때 0으로, z가 아주 큰 양수일때 1로 바꿔주는 함수
    - 흑백논리

    <img src="./img/ml0005.png" width = 600>
...
- SGD 설명 정리
    ...
    - 데이터를 일정 간격으로 뛰어넘으면서 손실함수 최소값 찾는 횟수를 줄이는 것

- 에포크(epoch)
    - SGD로 훈련세트를 한번 다 사용한 과정. 반복횟수
    - 같은 훈련세트를 매번 셔플해서 반복학습한다.
...
- `손실함수` - 머신러닝 알고리즘이 얼마나 엉터리인지 측정하는 기준 함수
    ...
    - 로지스틱 손실함수(이진 크로스엔트로피) 또는 로그 손실
        ...
        <img src ="./img/ml0006.png" width=400>
...
- 프로세스 : 하나 학습 -> 손실함수 기반으로 평가 -> 파라미터 조정 -> 다시 학습 ....
